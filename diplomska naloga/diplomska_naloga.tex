...
pa ne le za raziskave, pri katerih so podlaga statistični podatki in analize, ampak tudi za vrednotenje hipotez in dokazov sodnega procesa. Čedalje 
...
oblike Bayesovega izreka za merilo vrednosti dokazov uporabljamo razmerje verjetij.\\
...
\slovar{
 ...
 \geslo{defence attorney's fallacy}{zmota obrambnega odvetnika}
 \geslo{network}{omrežje}
 \geslo{prosecution's proposition}{predlog tožilstva}
 \geslo{defence's proposition}{predlog obrambe}  
}
...
\nocite{ENFSI,collins,byers,aitkena,hoff,aitkenb,aitkenc,aitkend,bello,berger,conklin,dahlman,fentona,fentonb,finkelstein,gastwirth,giannini,glover,iliinski,maarcot,macedo,maddan,matthews,orban,schumana,schumanb,scurich,balaba,blankenship,skorupski,balding,bidermann,tarling}
...
postane sporno, saj lahko takšni pretirani posegi ovirajo statistike pri izračunu verjetnostnega vpliva spremenljivk. Odvetniki sicer
...
do napačnih zaključkov in nepravilnih odločitev v sodnih procesih. Zagotovo določitev spremenljivk ne sme biti naloga le odvetnikov ali le statistikov, 
ampak menim, da je sodelovanje med statistiki in odvetniki pomembno, saj vsak prispeva svoj del poznavanja teorije,
...
statistikov, ki ocenijo zanesljivost podatkovne baze in pravilno razlagajo rezultate statistične analize. Pred pričanjem na sodišču moramo
...
mnenju bi se na tem mestu že lahko vprašali oziroma soočili s problemom, ali je izbrana mera jasen kazalnik zadevnega koncepta.\\
Zanesljivost je stopnja, do katere je spremenljivka dosleden in zanesljiv kazalnik koncepta. Spremenljivka je namenjena merjenju teh opazovanj ali konceptov. 
Običajno ima več kot eno možno vrednost. Merjenje spremenljivke mora biti jasno opredeljeno oziroma mora imeti operativno opredelitev. \\
Hipoteza je izražena v obliki odnosa med spremenljivkami. V smislu reševanja problemov hipoteza opisuje način, na katerega je mogoče rešiti problem. \\
V raziskavi neodvisna spremenljivka ($X$) povzroča učinek ali vpliv na odvisno spremenljivko ($Y$). Odvisna spremenljivka ($Y$) se lahko spremeni zaradi prisotnosti 
...
raziskovalno zasnovo izolirati učinek ukrepa na problem. \\
Klasični eksperiment vključuje razvrstitev udeležencev v eksperimentalno in kontrolno skupino. Ključni element postopka je naključna izbira, ki zagotavlja, 
da sta skupini primerljivi v vseh pomembnih vidikih. V statističnem smislu naključna dodelitev pomeni, da ima vsak član ciljne populacije enake možnosti, da 
bo izbran v eksperimentalno skupino. Drugi element je verjetnostno vzorčenje. \\
Statistik običajno ne more preučiti vseh elementov populacije. Večina raziskav se izvaja z izbiro vzorca iz populacije. \\
...
\begin{definicija}
    Naj bo $H$ neki dogodek in $\bar{H}$ negacija oziroma komplement dogodka $H$. Dogodka $H$ in $\bar{H}$ sta znana kot komplementarna dogodka.
\end{definicija}
...
osebi ali obtožencu. Pri presoji dokazov se uporablja različne metode in tehnike, ki temeljijo na statistični verjetnosti. 
...
Ali je obtoženec kriv glede na znan dokaz $E$, je glavna stvar, ki nas pri sojenju zanima. Če imamo torej na voljo dokaz $E$, nas zanima pogojna 
verjetnost
\[
...
$W_m$ \dots dokaz, tj. poročanje priče, da je bil taksi moder,\\
$W_z$ \dots dokaz, tj. poročanje priče, da je bil taksi zelen. \\\\
...
Mnenje in poročila statistikov naj bi bila ključni vir informacij, ki lahko prispevajo k objektivnemu in strokovnemu 
...
Marca 2003 je bila medicinska sestra Lucia de Berk (v nadaljevanju osumljenka) v Haagu na Nizozemskem obsojena na dosmrtni zapor zaradi obtožbe 
uboja več bolnikov (v nadaljevanju je zadevni dogodek smrt bolnika) v dveh bolnišnicah, v katerih je delala v bližnji preteklosti (bolnišnici JZK in RKZ). V bolnišnici JKZ 
...
\begin{table}[h!]
    ...
    \label{table:1}
     \begin{tabular}{l c c c}
        \hline
        Ime bolnišnice (in številka izmene) & JKZ  & RKZ-1 & RKZ-2 \\ 
        \hline
     ...
     \end{tabular}
 \end{table}
Tekom sodbe je bilo ugotovljeno, da je osumljenka v RKZ dejansko opravila tri izmene in ne le ene, zato v kasnejših izračunih uporabimo to 
pravilno število.\\\\
...
\begin{enumerate}
    ...
    \item smrti bolnikov se pojavljajo neodvisno drug od drugega.
\end{enumerate}
...
potem je pogojna verjetnost, da je bila osumljenka priča $x$ zadevnim dogodkom, če se je zgodilo $k$ smrti bolnikov, naslednja 
...
so različni dokazi med seboj povezani. Z določitvijo predhodnih verjetnosti lahko pridemo do zelo različnih rezultatov, nimamo pa več težav s 
...
predhodnih verjetnosti in ocenimo njihov vpliv na verjetnost dogodka $S$ in njegove negacije. Nato moramo določiti vrednost $P(M \lvert S)$, ki
je običajno enaka ena - če bi obtoženec dejansko pustil sledove, bi laboratorijske analize pokazale ujemanje, kar imenujemo lažni
...
$\frac{1}{f}$, je to milijarda. S formulo pa dobimo:
\[
...
Vpliv stopnje laboratorijskih napak kaže, da ne glede na to, kako nizka je pogostost profila,
...
Ker profil DNK predstavlja del naše genetske zasnove in imajo ljudje, ki so v sorodu, večjo verjetnost, da bodo imeli enak profil DNK, kot ljudje,
...
Relativne frekvence vedno navajajo ali predpostavljajo, da obstaja neki
...
Po metodah, ki temeljijo na naravnih frekvencah, je potrebno izračunati, koliko ljudi z zadevnim profilom DNK je v populaciji osumljencev, tako
da se $f$ pomnoži z $m$.  \\
...
Z binomsko porazdelitvijo lahko izračunamo verjetnost, da se bo dogodek $X$, na primer profil DNK, pojavil $k$ - krat v $s$ - kratnem
številu neodvisnih ponovitev, pri čemer ima dogodek $X$ frekvenco $f$. Želimo vedeti, kolikšna je verjetnost, da ima točno en posameznik ustrezen DNK
...
\begin{table}[h!]
    ...
    \begin{tabular}{l l c c}
        ...
        \hline
        10 milijonov & 1 v 100 milijonov & 0,9 & 0,9 \\
        100 milijonov & 1 v 100 milijonov & 0,62 & 0,5 \\
        1 milijarda & 1 v 100 milijonov & 0 & 0,09 \\ \hline
...
upoštevati številnih zapletov, kot je vpliv stopnje laboratorijskih napak. Bayesovo pravilo namreč omogoča upoštevanje različnih dejavnikov in 
...
izbira metode odvisna od specifičnih zahtev problema in od tega, katera metoda je najbolj primerna za njegovo reševanje.\\\\
...
žrtev. Kljub temu imajo ti modeli notranje pomanjkljivosti, ki jih ni mogoče zanemariti. \\
...
anomalij pri uporabi drugih sredstev in storitev. Na primer, če priča trdfi, da je videla osumljenca na kraju zločina, sodišče ne more oceniti verjetnosti, 
...
ustrezni DNK profil. Ta verjetnost je enaka pogostosti profila DNK. Težava tega pristopa je, da je verjetnost naključnega ujemanja lahko predstavljena
...
\begin{enumerate}
   ...
   \item zato mora biti verjetnost, da je vir sledi DNK s kraja zločina obtoženec zelo velika, ampak je obratna od 1 proti 100 milijonov.
\end{enumerate}
Takšno sklepanje je napačno in je znano kot tožilčeva zmota. \\
Ekvivalentna je enačbi
...
Nato sledi (3.) korak, saj če je $P(M \lvert \neg S) = f$ in $P(\neg S \lvert M) = f$, potem je $1-f = P(S \lvert M)$. Z uporabo Bayesovega 
pravila izognemo možnosti te napake in omogočimo popolnejšo analizo. Namesto verjetnosti naključnega ujemanja forenzični strokovnjaki pogosto pričajo o razmerju verjetij dokazov DNK, in
sicer kot:
\[
   \frac{P(M \lvert S)}{P(M \lvert \neg S)}. \vspace{2mm}
\]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Opredelitev}
V Bayesovi formuli \eqref{eq:bpravilo} nadomestimo $H$ s komplementom tega dogodka, $\bar{H}$, in enakovredna različica Bayesovega izreka je
\begin{equation}
   P(\bar{H} \lvert E) = \frac{P(E \lvert \bar{H})P(\bar{H})}{P(E)}, \vspace{2mm}
\end{equation}
kjer $P(E) \ne 0$.\\\\
Če prvo enačbo delim z drugo, dobimo Bayesov izrek v obliki razmerja verjetij
\begin{equation}
   \frac{P(H \lvert E)}{P(\bar{H} \lvert E)} = \frac{P(E \lvert H)}{P(E \lvert \bar{H})} \times \frac{P(H)}{P(\bar{H})}. \vspace{2mm}
\end{equation}
Leva stran enačbe je razmerje verjetij. Pogojna verjetnost na desni strani dogodka, $H$ in $\bar{H}$,
sta v števcu in imenovalcu različna, medtem ko je dogodek $E$, katerega verjetnost nas zanima, enak. Desni faktro na desni strani enačbe 
predstavlja verjetnost v korist dogodka $H$ brez kakršnihkoli informacij o dogodku $E$.\\
\begin{definicija}
    Razmerje
    \begin{equation}
...
Oblika Bayesovega izreka nato omogoča, da se predhodna razmerja, tj. pred predstavitvijo obravnavanih dokazov $Ev$, v korist krivde posodobijo v 
aposteriorna razmerja ob upoštevanju $Ev$, na naslednji način:
\[
   \frac{P(H_p \lvert Ev)}{P(H_d \lvert Ev)} = \frac{P(Ev \lvert H_p)}{P(Ev \lvert H_d)} \times \frac{P(H_p)}{P(H_d)}. \vspace{2mm}
\]
Ker ima vsak kazenski primer specifične posebnosti, moramo v enačbo vpeljati še t.i. informacije o ozadju, $I$. Te informacije so osebne za vsakega 
posameznika, zato se lahko zgodi, da je vrednost posameznih dokazov drugačna za vsakega posameznika. Če jih ne upoštevamo, lahko posamezno vrednotenje dokazov 
postane preveč splošno. Ob upoštevanju informacij o ozadju $I$ dobimo zapis
\[
...
\]
Pri vrednotenju dokazov $Ev$ sta potrebni dve verjetnosti - verjetnost dokazov, če je obtoženec kriv in glede na informacije o ozadju, ter
verjetnost dokazov, če je obtoženec nedolžen in glede na informacije o ozadju. Informacije o ozadju so včasih znane kot okvir okoliščin
...
Če imamo dva konkurenčna dokaza oziroma hipotezi, na primer $H_p$ proti $H_d$, ter vrednost V, se velikokrat uporabljajo kvantitativne lestvice 
inštituta ENFSI za poročanje o vrednosti $V$ podpore dokazov (za ponazoritev glej tabelo \ref{table:1}).
Pri uporabi takšnih tabel moramo biti previdni, če so hipoteze predlagane na podlagi podatkov; v tem primeru postanejo smiselne le, če 
podamo predhodne verjetnosti za obravnavane hipoteze.

\begin{table}[h!]
    \centering
    \caption{Kvalitativna lestvica za poročanje o vrednosti $V$ podpore dokazov za $H_p$ proti $H_d$. Vir: ENFSI, 2015 \vspace{2mm}}
    \label{table:1}
...
Denimo, da želimo izmeriti vrednost $V$ dokazov $E$ v prid krivdi $H_p$. Pri tem naj bi obstajala odvisnost od osnovnih informacij $I$, vendar ta ni izrecno
navedena. Predpostavimo, da je vrednost $V$ odvisna samo od verjetnosti dokazov $E$ ob pogoju, da je obtoženec kriv ($H_p$), in od verjetnosti dokazov $E$ ob pogoju,
da je obtoženec nedolžen ($H_d$). Naj bo $x=P(E \lvert H_p)$ in $y=P(E \lvert H_d)$. Zgornja predpostavka pravi, da je $V = f (x, y)$ za neko funkcijo $f$.
Vzemimo še en dokaz $T$, ki je neodvisen od dokazov $E$ in od $H_p$ (in s tem $H_d$) in je tak, da je $P(T) = a$. Nato
\begin{align}
   P(E,T \lvert H_p) & = \\
   ...
   & = a x, \vspace{2mm}
\end{align}
pri čemer iz (12.4) v (12.5) upoštevamo neodvisnost dokaza $E$ in $T$ ter iz (12.5) v (12.6) neodvisnost dokaza $T$ in dogodka $H_p$. Podobno
\[
   P(E,T \lvert H_d)  = a y. \vspace{2mm}
\]
Vrednost kombiniranih dokazov $(E, T)$ je enaka vrednosti $E$, saj je bil $T$ predpostavljen kot nepomemben. Vrednost $(E, T)$ je $f(a x, a y)$ in 
vrednost $E = f (x, y)$ in $V = f (x, y)$. Tako je $f(a x, a y) = f(x,y)$ za vsak $a$ na intervalu $[0,1]$ možnih vrednosti $P(T)$.
Če definiramo $f(x, y) = \frac{x}{y}$ sledi
\[
   f(x,y) = f \big(\frac{x}{y}\big)
   f(a x, a y) = f(\frac{a x}{a y}) = f(\frac{x}{y}). \vspace{2mm}
\]
Izkaže se, da je to edina možnost in je zato $V$ funkcija razmerja verjetij
\[
   \frac{P(E \lvert H_p)}{P(E \lvert H_d)}. \vspace{2mm}
\]\\
Razmerje verjetij nam pove, katera hipoteza je bolje podprta z dokazi. Kadar sta hipotezi medsebojno izključujoči, nam pove še več.
...
Če je le mogoče, je treba upoštevati verjetnosti za vse razumne alternativne hipoteze (tako da je nabor hipotez izčrpen). Če obravnavamo samo 
...
na poznavanju primera. \\
Drugi viri negotovosti vključujejo pridobivanje predhodnih verjetnosti, pogojenih z razpoložljivim znanjem, ali celo
...
vrednosti dokaza. \\
V praksi je za kriminalistično preiskavo na voljo en niz
...
Združene države Amerike, kjer se lahko Bayesovo pravilo šteje kot poseg v pravico porote: poroti naj ne bi bilo potrebno govoriti, 
...
(tj. enostavna hipoteza). Vendar pa lahko pride do primerov, ko se primerjajo sestavljene hipoteze. V takem primeru je Bayesov faktor razmerje dveh 
...
\begin{equation}\label{eq:rv}
    V = \frac{P(E \lvert H_p)}{P(E \lvert H_d)} = e^{\mu r_l-\mu _L r_l}\biggl(\frac{\mu _L r_l}{\mu r_l}\biggr)^{k_l} = 
    e^{\mu r_l-\mu _L r_l}\biggl(\frac{\mu _L}{\mu}\biggr)^{k_l}.\vspace{2mm}
\end{equation}
Za oceno izida katerega koli izračuna s tem razmerjem lahko iz tabele \ref{table:1} izpeljemo naslednjo lestvico za opis velikosti razmerja verjetij.
\begin{table}[h!]
    \centering
    ...
    \label{table:2}
     \begin{tabular}{l l}
        ...
        $V >$ 10000 & Dokazi močno podpirajo $H_p$. \\ [1ex]
        \hline
     \end{tabular}
 \end{table} 
...
tudi \textit{verjetnost naključnega ujemanja}. V našem vzročno-posledičnem okviru je $F$ (lastnosti) enakovredna bolj formalno opredeljeni verjetnosti
$P(C \lvert \neg B)$, tj. verjetnost, da se oseba, ki ni vpletena v kaznivo dejanje, ujema z dokazi kaznivega dejanja.\\\\
...
tj. $P(C \lvert \neg B)$, s $P(\neg A \lvert C)$, tj. verjetnost, da obtoženec ni storil kaznivega dejanja, ampak se dokazi vseeno ujemajo 
z dokazi kaznivega dejanja. Presega napako predhodne verjetnosti, saj si jo lahko 
predstavljamo kot dopolnitev te napake z dodatno napačno predpostavko $P(A) = P(B)$.\\\\
\textit{Napaka verjetnosti $P(\text{drugo ujemanje})$}: gre za zmoto, ko verjetnost, da se oseba, ki ni vpletena v kaznivo dejanje, ujema 
z dokazi kaznivega dejanja, tj. $P(C \lvert \neg B)$, enačimo z verjetnostjo (imenujmo jo $q$), da vsaj en nedolžen član populacije 
ustreza dokazom. Posledica te napake je običajno močno pretiravanje z vrednostjo dokaza $C$.\\\\
...
\textit{Napaka pri številčnem preračunavanju}: pri tem gre za zamenjavo verjetnosti, da se oseba, ki ni vpletena v kaznivo dejanje, 
ujema z dokazi kaznivega dejanja, tj. $P(C \lvert \neg B)$, s pričakovanim številom drugih oseb,
...
informiranje verjetnosti $P(A \lvert D)$, kjer $D$ pomeni, da test določi, ali se dokazi obtoženca ujemajo z dokazi kaznivega dejanja. Napaka 
je, da ne upoštevamo $P(D \lvert \neg A)$. Če je $P(D \lvert A) \leq P(D \lvert \neg A)$, potem dokaz nima vrednosti.\\\\
...
pogojnih verjetnosti in vprašanj večkratnega testiranja. Pogosto se pojavlja v kazenskem pravu, vendar jo pogosto ne prepoznajo, deloma zato, ker 
preiskovalci nimajo močne intuicije o tem, kaj zmota sploh pomeni. 
Verjetnostno utemeljevanje pravnih dokazov se torej skrči na preprost vzročni scenarij: začnemo z neko hipotezo $H$ in opazujemo neki dokaz $E$.
...
tožilčeva zmota.\\\\
Za lažjo predstavo si oglejmo primer. Naj se krvna skupina obtoženca ujema s krvjo storilca kaznivega dejanja. Ta krvna skupina je tako redka, 
...
res, da obstaja le verjetnost 1 proti 1000, da se krvna skupina naključne osebe ujema s krvjo zločinca, je verjetnost, da je oseba, ki ustreza redkemu 
...
\begin{figure}[!ht]\label{fig:slika1}
   ...
   \caption{Populacija, predstavljena s 100 kvadrati, z 9 črtastimi, 25 sivimi in 4 črtastimi in sivimi. (Vir: \cite{iliinski})}\vspace{2mm}
\end{figure}\\
...
\begin{figure}[!ht]\label{fig:slika2}
   ...
   \caption{Populacija, predstavljena s 100 kvadrati, z 16 črtastimi, 16 sivimi in 4 pikčastimi in sivimi. (Vir: \cite{iliinski})}\vspace{2mm}
\end{figure}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Načini za izogib zmotam}
V zadnjih letih sta statistika in verjetnost v kazenskem pravu napredovali. Sodniki, tožilstvo in porota se zavedajo nerazumevanja teh znanosti, zato 
...
lahko priznanje zmanjša verjetnost krivde. Vprašanje je, kdaj natančno je priznanje znak krivde in kdaj ne. Priznanje poveča verjetnost krivde 
...
naslednji bolj celovit način, pri čemer $I$ predstavlja dogodek \textit{je bil zaslišan}:
...
\end{equation}
Da bi videli, zakaj je prvo razmerje na desni strani \eqref{eq:aposteriorne} enakovredno produktu prvih dveh razmerij na desni strani \eqref{eq:razširitev}, najprej upoštevajmo naslednji dve osnovni enakosti:
...
\begin{equation}\label{eq:ver2}
    P(\neg H \lvert I) = \frac{P(\neg H) \times P(I \lvert \neg H)}{P(I)}.
\end{equation}
Če zdaj \eqref{eq:ver1} delim s \eqref{eq:ver2}, dobim
...
izračuni naj bi poseldično prikazovali, da je velika verjetnost, da obstaja povezava med izmeno osumljenke in pojavom incidenta. Ti sklepi sodišča bi morali statistikom 
vzbujati dvome, saj je sodba dvoumna in lahko bi rekla, da je sodišče storilo znano tožilčevo zmoto. Po sklepu sodišča bi lahko rekli, da govorijo o verjetnosti, da se je nekaj zgodilo 
ob predpostavki, da je vse popolnoma naključno ali pa si sodbo sodišča razlagamo kot verjetnost, da se je nekaj naključno zgodilo. Ti dve trditvi pa sta različni, kar lahko 
...
Omogočajo, da se lotimo kompleksnejših verjetnostnih analiz, kot bi bilo to mogoče s tradicionalnimi pristopi, kar pride še posebej prav pri primerih z ogromno dokazi. \\\\
...
in zato vključujejo le podskupino dokazov. Če se za izdelavo ne uporablja dosleden okvir, lahko Bayesova omrežja, ki jih oblikujejo različne stranke za en
primer, kažejo različne rezultate. Pri oblikovanju Bayesovega omrežja za pravno sklepanje je ključnega pomena, da se oblikuje omrežje, ki je razumljivo poroti
in sodniku.\\\\
...
analize, bi po mojem mnenju tu morali biti bolj previdni in natančni. Torej do napačnih rezultatov lahko pridemo še pred predstavitvijo analize na sodišču, kar
...
Ko statistik predstavi analizo in rezultate na sodišču, pa se lahko zgodijo tudi druge zmote. Podrobno sem opredelila Tožilčevo zmoto, ki je tudi
najpogostejša. Največkrat se verjetno zgodi, zaradi pomanjkljive predstavitve in razlage analize in, po mojih ugotovitvah, nezanimanja porote in sodnika, ko se predstavi
neki algebrski zapis izračuna. Zaradi zamenjave dveh različnih pogojnih verjetnosti se zgodijo tudi napačni zaključki sodnih procesov. Zmotam, ki izhajajo iz napačnega
...
Glavna ugotovitev dela je, da se lahko s pomočjo Bayesovih omrežij izognemo zmotam, saj ne prikažejo polne algebrske oblike verjetnostne formule, omogočajo pa skoraj popolno
avtomatizacijo potrebnih verjetnostnih izračunov in več prefinjenih možnosti. Ponovno pa je pomembno, da se za izdelavo uporablja dosleden okvir, saj lahko Bayesova omrežja,
ki jih oblikujejo različne stranke za en primer, kažejo različne rezultate. Ker pa je v praksi ponavadi v sodni proces vključeno ogromno dokazov, ki jih težko analiziramo s
